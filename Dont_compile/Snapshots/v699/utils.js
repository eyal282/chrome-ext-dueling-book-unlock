...
function addScrollBars() {
	...
	(function ($) {
		...
		$.fn.scrollLeft = function(value) {
			if (arguments.length >= 1) {
				...
				//}
				if (this.hasClass("scrollpane") && this.find("*[data-overlayscrollbars-viewport]:first").length > 0) {
					return this.data("scrollpane").elements().viewport.scrollTo({"left":value});
				}
				return _scrollLeft.call(this, value); 
			}
			if (this.hasClass("scrollpane") && this.find("*[data-overlayscrollbars-viewport]:first").length > 0) {
	...
	*/
	...
	$('.scrollpane').each(function(i,e){
		...
		/*$(e).find('.os-scrollbar .os-scrollbar-track').css("background-color", "rgb(255, 255, 255, 0.3)");
		//$(e).find('.os-scrollbar-vertical').css("max-height", $(e).height());
		...
		//$(this)[0].removeEventListener("pointerenter");
		//$(this)[0].removeEventListener("pointerdown");
		//$(this)[0].removeEventListener("pointerleave");
		
		
	});
	
	/*
	$('.scrollpane').mouseenter(function(e){
	
		e.preventDefault();
		e.stopPropagation();
		console.log('mouseover');
		//TweenMax.killTweensOf($(this).find('.os-scrollbar .os-scrollbar-track'));
		console.log($(this).find('.os-scrollbar .os-scrollbar-track'));
		TweenMax.to($(this).find('.os-scrollbar .os-scrollbar-track'), 0.7, {"backgroundColor":"rgb(255, 255, 255, 0.8)"});
		if ($(this).hasClass("list") || $(this).hasClass("textarea")) {
			TweenMax.to($(this).find('.os-scrollbar-handle'), 0.5, {alpha:1});
		}
	...
	$('.scrollpane').mouseleave(function(e){
		e.preventDefault();
		e.stopPropagation();
	...
	*/
	...
	$('.scrollpane').mouseleave(function(){
	...
	/*$('.scrollpane').each(function(i,e){
		$(this)[0].addEventListener('pointerenter', function(event) {
		...
		$(this)[0].addEventListener('pointerleave', function(event) {
			event.preventDefault();
			event.stopPropagation();
...
//$('.scrollpane').filter(function(){ return $(this).hasClass("selectable") })
...
function getLink(title, body, str, callback) {
	hideDisplayBoxes();
	$('#link').show();
	$('#link .title_txt').text(title);
	$('#link .body_txt').text(body);
...
function exportDeckYDKE() {
	...
	var url = toURL(obj);
	getLink("YDKE Code", "Here is the code for this decklist", url, function(){
		if (!username) {
			return
		}
