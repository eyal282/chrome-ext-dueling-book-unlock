...
function editStats(player, data) {
	...
	var prev_atk = card.data("new_atk") ?? card.data("cardfront").data("atk");
	var prev_def = card.data("new_def") ?? card.data("cardfront").data("def");
	var atk_difference = ~~data.atk - ~~prev_atk;
	var def_difference = ~~data.def - ~~prev_def;
	var txt = getStatsTextByCard(card);
	if (data.atk != "?" || data.def != "?") {
		if (!txt.text().startsWith(data.atk) || card.data("cardfront").data("monster_color") != "Link" && !txt.text().endsWith("/" + data.def)) {
			var tween = TweenMax.to({}, 1, {ease:Expo.easeOut, onUpdate:function(){
				txt.html(getATKColor(card, 
				data.atk == "?" ? data.atk : ~~prev_atk + Math.floor(tween.progress() * atk_difference), 
				data.def == "?" ? data.def : ~~prev_def + Math.floor(tween.progress() * def_difference)));
			}, onComplete:function(){
...
function getStatsTextByCard(card) {
	var txt = null;
	switch(card) {
		case player1.m1:
			txt = hm1_txt;
		...
		if (zones[i]) {
			...
			if (zones[i].data("cardfront").data("atk")) {
				...
				if (zones[i].data("face_down") && !show_cards && !duelist || duelist && zones[i].data("face_down") && !isPlayer1(zones[i].data("controller").username) || TweenMax.isTweening(zones[i])) {
					...
					texts[i].html(getATKColor(zones[i], zones[i].data("new_atk"), zones[i].data("new_def")));
...
function getATKColor(card, new_atk, new_def) {
	var str = "";
	if (!new_atk || new_atk == card.data("cardfront").data("atk")) {
		str += card.data("cardfront").data("atk");
	}
	else {
		str += '<font color="#' + (~~new_atk > ~~card.data("cardfront").data("atk") ? "33FFFF" : "FF3300") + '">' + new_atk + '</font>';
	}
	if (card.data("cardfront").data("monster_color") != "Link") {
		if (!new_def || new_def == card.data("cardfront").data("def")) {
			str += '/' + card.data("cardfront").data("def");
		}
		else {
			str += '<font color="#' + (~~new_def > ~~card.data("cardfront").data("def") ? "33FFFF" : "FF3300") + '">/' + new_def + '</font>';
		}
	}
	return str;
}
...
function banish(player, data) {
	...
	var host = card.hasClass("xyzmaterial") ? getXyzHost(card.data("id")) : null;
	var card = removeCard(player, data);
	if (card) {
		card.data("face_down", false);
	}
	...
	playSound(Banish);
	if (host) {
		updateXyzMaterials(host.data("controller"), host);
	}
	updateViewing(); // 5-24-23
}
...
function detachAllMaterials(card) {
	while (array(card.data("xyz_arr")).length > 0) {
		...
		material.removeClass("xyzmaterial");
			//material.off("click", viewMaterials);
			//material.css("cursor", "auto");
			//material.find('.content:first').css("cursor", "auto");
...
function getFieldCard(player, data) {
	var field_arr = [player.m1, player.m2, player.m3, player.m4, player.m5, player.s1, player.s2, player.s3, player.s4, player.s5, player.fieldSpell, player.pendulumLeft, player.pendulumRight, linkLeft, linkRight, player.skillCard];
	for (var card of field_arr) {
		if (card == null) {
			continue;
		}
...
function replaceCard(player, start_card, end_card) {
	var field_arr = [player.m1, player.m2, player.m3, player.m4, player.m5, linkLeft, linkRight];
	for (var card of field_arr) {
		if (card == end_card) {
			continue;
		}
		var zone = getZoneByCard(player, end_card);
		removeFromField(player, {"id":end_card.data("id")});
		getNextMonsterZone(player, start_card, {"zone":zone});
		updateController(player, start_card);
		return;
	}
}
...
function updateXyzMaterials(player, card, end, fast) {
	...
	for (var i = card.data("xyz_arr").length - 1; i > -1; i--) {
		...
		material.addClass("xyzmaterial");
		material.removeClass("target");
		//material.css("cursor", "auto");
		//material.find('.content:first').css("cursor", "pointer");
		//material.off("click", viewMaterials);
		//material.on("click", viewMaterials);
	...
	for (var i = 0; i < arr.length; i++) {
		...
		for (var j = 0; j < arr[i].data("xyz_arr").length; j++) {
			if (arr[i].data("xyz_arr")[j].data("id") == data.id) {
				var material = arr[i].data("xyz_arr").splice(j, 1)[0];
				material.removeClass("xyzmaterial");
					//material.off("click", viewMaterials);
					//material.css("cursor", "auto");
					//material.find('.content:first').css("cursor", "auto");
...
function newCard(player, data) { 
	...
	card.data("controller", null); // 7-13-23 Not having a controller is a disaster. Why was this line here? // it was only a disaster for saveGameState, which isn't even functional						
...
/*function getCardByZone(player, str) {
	if (str == "M-1") {
		return player.m1;
	}
	if (str == "M-2") {
...
function getCardByZone(player, str) {
	switch(str) {
		...
		case "F-2":
			return player.opponent.fieldSpell;
		case "Left Extra Monster Zone":
			return isPlayer1(player.username) ? linkLeft : linkRight;
		case "Right Extra Monster Zone":
			return isPlayer1(player.username) ? linkRight : linkLeft;
	}
	return null;
}

function getZoneByCard(player, card) {
	switch(card) {
		case player.m1:
			return "M-1";
		case player.m2:
		...
		case player.m5:
			return "M-5";
		case player.opponent.m1:
			return "M2-1";
		case player.opponent.m2:
			return "M2-2";
		case player.opponent.m3:
			return "M2-3";
		case player.opponent.m4:
			return "M2-4";
		case player.opponent.m5:
			return "M2-5";
		case player.s1:
			return "S-1";
		case player.s2:
		...
		case player.s5:
			return "S-5";
		case player.fieldSpell:
			return "F-1";
		case player.opponent.fieldSpell:
			return "F-2";
		case linkLeft:
			return isPlayer1(player.username) ? "Left Extra Monster Zone" : "Right Extra Monster Zone";
		case linkRight:
			return isPlayer1(player.username) ? "Right Extra Monster Zone" : "Left Extra Monster Zone";
...
function countdownE() {
	players.forEach(function(p,i){
		if (isOnline(p.username) || !p.seconds) {
			return;
		}
...
function viewMaterials(card) {
	viewing_card_with_materials = getXyzHost(card.data("id"));
	viewingE("Xyz Materials");
	if (Duelist()) {
		Send({"action":"Duel", "play":"View materials", "card":$(this).data("id")});
	}
}

function getXyzHost(card_id) {
	var field_arr = [player1.m1, player1.m2, player1.m3, player1.m4, player1.m5, player2.m1, player2.m2, player2.m3, player2.m4, player2.m5, linkLeft, linkRight];
	for (var monster of field_arr) {
		if (monster == null) {
			continue;
		}
		if (monster.data("id") == card_id) {
			return monster;
		}
		for (var material of monster.data("xyz_arr")) {
			if (material.data("id") == card_id) {
				return monster;
			}
		}
	}
