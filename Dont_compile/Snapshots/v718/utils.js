...
var zooming = false;
//var Zoom = 100; // if someone's "correct" zoom is 125, they'll get a notification at the start screen if this starts at 100
var Zoom = getZoom(); // and now someones not notified at all at the start
...
addButton($('#link .ok_btn'), hideDisplayBoxes);
...
addButton($('#recaptcha_box .cancel_btn'), cancelCallback);
...
jQuery.fn.extend({
	...
	scrollMax: function() {
		var maxVSP = $(this)[0].scrollHeight - $(this).outerHeight();
		if (this.hasClass("scrollpane") && this.find("*[data-overlayscrollbars-viewport]:first").length > 0) {
			maxVSP = this.find("*[data-overlayscrollbars-viewport]:first")[0].scrollHeight - this.find("*[data-overlayscrollbars-viewport]:first").outerHeight();
		}
		return maxVSP;
	},
	scrollLeftMax: function() {
		var maxVSP = $(this)[0].scrollWidth - $(this).outerWidth();
		if (this.hasClass("scrollpane") && this.find("*[data-overlayscrollbars-viewport]:first").length > 0) {
			maxVSP = this.find("*[data-overlayscrollbars-viewport]:first")[0].scrollWidth - this.find("*[data-overlayscrollbars-viewport]:first").outerWidth();
		}
		return maxVSP;
	},
	scrollHeight: function() {
		if (this.hasClass("scrollpane") && this.find("*[data-overlayscrollbars-viewport]:first").length > 0) {
			return this.find("*[data-overlayscrollbars-viewport]:first")[0].scrollHeight;
		}
		return this[0].scrollHeight;
	},
});
...
function addScrollBars() {
	...
	(function ($) {
		...
		$.fn.parent = function() {
			...
			if (p.is("[data-overlayscrollbars-viewport]")) {
	...
	(function ($) {
		...
		$.fn.append = function(value) {
			if (this.hasClass("scrollpane") && this.find("*[data-overlayscrollbars-viewport]:first").length > 0) {
				return _append.call(this.find("*[data-overlayscrollbars-viewport]:first"), value); 
			}
	...
	(function ($) {
		...
		$.fn.prepend = function(value) {
			if (this.hasClass("scrollpane") && this.find("*[data-overlayscrollbars-viewport]:first").length > 0) {
				return _prepend.call(this.find("*[data-overlayscrollbars-viewport]:first"), value); 
			}
	...
	(function ($) {
		...
		$.fn.html = function(value) {
			if (this.hasClass("scrollpane") && this.find("*[data-overlayscrollbars-viewport]:first").length > 0) {
				if (arguments.length >= 1) {
					return _html.call(this.find("*[data-overlayscrollbars-viewport]:first"), value); 
				}
				return _html.call(this.find("*[data-overlayscrollbars-viewport]:first")); 
			}
			if (arguments.length >= 1) {
	...
	(function ($) {
		...
		$.fn.text = function(value) {
			if (arguments.length >= 1) {
				if (this.hasClass("scrollpane") && this.find("*[data-overlayscrollbars-viewport]:first").length > 0) {
					return _text.call(this.find("*[data-overlayscrollbars-viewport]:first"), value); 
				}
	...
	(function ($) {
		...
		$.fn.scrollTop = function(value) {
			if (arguments.length >= 1) {
				if (this.hasClass("scrollpane") && this.find("*[data-overlayscrollbars-viewport]:first").length > 0) {
					return this.data("scrollpane").elements().viewport.scrollTo({"top":value});
				}
	...
	(function ($) {
		...
		$.fn.scrollLeft = function(value) {
			if (arguments.length >= 1) {
				...
				//}
				if (this.hasClass("scrollpane") && this.find("*[data-overlayscrollbars-viewport]:first").length > 0) {
					return this.data("scrollpane").elements().viewport.scrollLeft({"left":value});
				}
	...
	$('#widget .body').removeClass("scrollpane");
	$('.scrollpane').each(function(i,e){
		//if (e == $('#view .content')[0]) {
		//	// With overlayScrollbars, the scroll position cannot seem to be set manually, making so the card menu is out of place // not sure if true anymore
		//	//return true;
		//}
		//if (e == $('#widget .body')[0]) {
		//	e.classList.remove("scrollpane");
		//	return true;
		//}
		var scrollpane = OverlayScrollbars(e, {
			//"resize":"none",
			...
			"scrollbars":{
				...
				//"visibility":"visible", // now causes horizontal scrollbars to show when they shouldln't exist
			},
			"overflow":{
				//"x":$(e).css("overflow-x") == "auto" ? "hidden" : $(e).css("overflow-x"),
				"x":$(e).css("overflow-x"),
				"y":$(e).css("overflow-y")
			},
			
		}, {
			//"updated":function(osInstance, onUpdatedArgs){
			//	//console.log(osInstance);
			//	//console.log(onUpdatedArgs);
			//},
			"scroll":function(osInstance, event){
				$(e).trigger("scroll");
			}
		});
		$(e).data("scrollpane", scrollpane);
		$(e).css("overflow", "hidden"); // needed for chrome pc
		//if ($(this).hasClass("selectable")) { // not sure why this is needed
...
function changeInputs(b) {
	...
	$(str).each(function(i,e){
		...
		div.css("width", parseInt(input.css("width")) - 6);
		div.css("height", (parseInt(input.css("height")) - 3) + "px"); /* needs to be a fixed number for Safari */
		div.css("display", input.css("display")); // untested
		div.css("opacity", input.css("opacity")); // untested
		div.data("proxy", input);
		div.insertAfter(input);
		input.data("proxy", div);
		input.css("opacity", 0);
		input.on("input blur", function(){
			if (input.attr("type") == "password") {
				input.data("proxy").text(toPassword(input.val()));
			}
			else {
				input.data("proxy").text(input.val());
			}
		});
...
function changeSelects(b) {
	...
	$(str + ", .comboboxlike").each(function(i,e){
		...
		if (!changing_components) {
			select.data("proxy", div);
			return true;
		}
		div.addClass("proxy unselectable");
		div.css("left", select.css("left"));
		div.css("top", select.css("top"));
		div.css("width", parseInt(select.css("width")) - 7);
		div.data("proxy", select);
		...
		div.css("display", select.css("display")); // untested
		...
		select.on('mouseout focusout', function(){
...
function changeButton(btn) {
	...
	var div = $('<div class="button proxy unselectable ' + (btn.hasClass("button2") ? "button2" : "") + '" style="left: ' + btn.css("left") + '; top: ' + btn.css("top") + '; right: ' + btn.css("right") + '; width: ' + (parseInt(btn.css("width")) - 2) + 'px; height: 17px; color: ' + btn.css("color") + '; display: ' + btn.css("display") + ';"></div>');
	if (!changing_components) {
		btn.data("proxy", div);
		return true;
	}
...
function changeButton(btn) {
	...
	btn.data("proxy", div);
	btn.css("opacity", 0);
	div.css("display", btn.css("display"));
	btn.on('mouseover focus', function(){
		div.addClass("button_selected");
	});
	btn.on('mouseout focusout', function(){
...
function changeCheckboxes(b) {
	...
	$(str).each(function(i,e){
		...
		if (!changing_components) {
			checkbox.data("proxy", div);
			return true;
		}
		div.data("proxy", checkbox);
		div.css("left", parseInt(checkbox.css("left")) + 2);
		div.css("top", parseInt(checkbox.css("top")) + 2);
		div.css("display", checkbox.css("display"));
		
		var check = $('<img class="check" src="' + IMAGES_START + 'svg/check.svg">');
		div.append(check);
		
		div.insertAfter(checkbox);
		checkbox.data("proxy", div);
		checkbox.css("opacity", 0);
		checkbox.on('mouseenter focus', function(){
			div.addClass("checkbox_selected");
		});
		checkbox.on('mouseout focusout', function(){
...
function changeRadios(b) {
	...
	$(str).each(function(i,e){
		...
		if (!changing_components) {
			radio.data("proxy", div);
			return true;
		}
		div.data("proxy", radio);
		div.css("left", parseInt(radio.css("left")) + 2);
		div.css("top", parseInt(radio.css("top")) + 2);
		div.attr("name", radio.attr("name"));
		div.attr("id", "radiobutton" + (i + 1));
		
		var img = $('<img class="check" src="' + IMAGES_START + 'svg/radio.svg">');
		div.append(img);
		
		div.insertAfter(radio);
		radio.data("proxy", div);
		radio.css("opacity", 0);
		radio.on('mouseenter focus', function(){
			div.addClass("radiobutton_selected");
		});
		radio.on('mouseout focusout', function(){
		...
		radio.change(function(){
			...
			$('input[type=radio][name="' + radio.attr("name") + '"]').not(radio).removeAttr("checked");
...
function CardFront() {
	...
	card.extend(card, {
		...
		initialize: function(id, name, treated_as, effect, pendulum_effect, card_type, monster_color, is_effect, type, attribute, level, ability, flip, pendulum, scale, arrows, atk, def, tcg_limit, ocg_limit, passcode, tcg, ocg, pic, custom, username, rush) {
			...
			//card.setCardName(); i'm moving it lower because it needs to be called after a skill card's name is populated
...
function getLink(title, body, str, html) {
	hideDisplayBoxes();
	$('#link').show();
	$('#link .title_txt').text(title);
	html ? $('#link .body_txt').html(body) : $('#link .body_txt').text(body);
	$('#link .input_txt').val(str);
	//$('#link .input_txt').select();
	$('#link .input_txt')[0].setSelectionRange(0, 0);
	$('#link .input_txt').focus();
}

function copyText(str, callback) {
	...
	navigator.clipboard.writeText(str).then(onComplete);
...
function getInput(title, str, placeholder, maxChars, callback, restrictedChars, displayaspassword, cancel_callback, tall) {
	...
	$('#input .input_txt').attr("type", displayaspassword ? "password" : "text");
	$('#input .input_txt').data("regex", restrictedChars ? restrictedChars : null);
...
//preview.find('.name_txt').css("transform")
//preview.find('.name2_txt').css("transform") 
...
function changeComponents(b) { // 1869 ms
	...
	changeButtons(b); // 1000
	console.timeEnd("changeButtons");
	changeCheckboxes(b); // 300
	changeRadios(b); // 0
	resizeComponents(b); // 600
...
function escapeHTML(str) {
	...
	str = replaceAll(str, "<", "&lt;");
		...
		//str = replaceAll(str, "'", "&apos;"); // added 7/16/23
		//str = replaceAll(str, ' ', "&nbsp;"); // added 6/15/23 to allow usernames with lots of spaces
	str = addBreaks(str)
	return str;
}
...
function takeScreenshot(event, usr) {
	...
	c.find("*").each(function(i,e){
		...
		if (e.parentNode.classList.contains("cell")) {
			//return true; // not needed 6/29/23
		}
		//if (e.className.indexOf && e.className.indexOf("os-content") >= 0) { // helps with lists // not convinced actually
		//if (e.className.includes("os-content")) {
		if (e.hasAttribute("data-overlayscrollbars-viewport")) { 
			//return true; // not needed 6/29/23
		}
		...
		if (!deleting) {
			var isCell = e.classList.contains("cell");
	...
	if (fancy_scrollbars) {
		c.find('.scrollpane').each(function(i,e){
			...
			var viewport = $(this).find("*[data-overlayscrollbars-viewport]:first");
			...
			viewport.css("top", -top2); // otherwise, it will just show content as is scrollTop was 0
			viewport.css("left", -left);
			scrollpane.find('.os-scrollbar-vertical').css("max-height", height);
			scrollpane.css("height", height + top);
			if (scrollpane.hasClass("list")) {
				scrollpane.css("clip-path", "inset(0px 0px " + ((height + top) - height) + "px 0px)"); // may not need to only be for lists
			}
			scroll_arr.push(function(){
				viewport.css("top", 0);
				viewport.css("left", 0);
				viewport.css("height", height);
				scrollpane.find('.os-scrollbar-vertical').css("max-height", "unset");
				scrollpane.css("height", height);
				scrollpane.css("clip-path", "unset");
		...
		//$('#online_users .users').scrollTop(80);
		//$('#online_users .users *[data-overlayscrollbars-viewport]:first').height()
...
function applyComputedStyles(element, styles) {
	console.log(styles);
	for (var property in styles) {
		if (styles.hasOwnProperty(property)) {
			//element.style[property] = styles[property];
			//var str = 'element.style.' + property + ' = ' + styles[property]; // element.style.0 = accent-color
			var str = 'element.style.' + styles[property] + ' = ' + property;
			console.log(str);
			eval(str);
		}
	}
...
function applyComputedStyles(element, styles) {
	Array.from(styles).forEach(function (key) {
		return element.style.setProperty(key, styles.getPropertyValue(key), styles.getPropertyPriority(key));
	});
}
...
function cssPath(e) {
	...
	while(true) {
		...
		str = ":nth-child(" + index + ")" + str;
		if (e.classList && Array.from(e.classList).length > 0) {
...
function getAvatarStart(pic) {
	...
	if (parseInt(pic.substring(0, pic.indexOf("."))) < 14000) {
		...
		pic = PROFILE_PIC_START + pic;
		if (pic.includes(".gif")) {
			pic = pic.replace('images.duelingbook.com', 'www.duelingbook.com/images');
		}
		return pic;
	}
}
...
function Cell(str) {
	...
	// ^ why was this done weirdly?
	username_txt.textContent = str;
	
	//username_txt.innerHTML = escapeHTML(str);
...
/*function Cell(str) {
	var cell = '<div class="cell cell1" onclick="cellClickE(this);" ondblclick="cellDoubleClickE(this);"><span style="opacity: ' + (streaming && isOffensive(str) ? 0 : 1) + '">' + escapeHTML(str) + '</span></div>';
	return $(cell)[0];
}*/
...
function cellClickE(e) {
	$(e.currentTarget).find('.cell.selected, .cell2.selected').removeClass("selected");
	$(e.target).filter('.cell').addClass("selected");
}



//onData({"action":"Online user","username":"     hungdywolf45"})


//$('span:contains(" hungdywolf45")').html()


//$('.status').eq(0).find('.status_txt')[0].innerHTML = "     hungdywolf45";
...
function exportDeckYDK() {
	...
	download(str, title + ".ydk", "text/plain");
}
...
function exportDeckYDKE() {
	...
	var url = toURL(obj);
	getLink("YDKe Code", 'Here is the code for this decklist. <font color="#0077FF"><a href="' + URL_START + 'welcome#importing" target="_blank"><u>Click Here</u></a></font> to learn how to use it.', url, true);
	copyText(url, function(){
		if (!username) {
			return
		}
		var notification = new Notification();
		notification.setupCopied("Successfully copied YDKe code to clipboard.");
...
function toURL(deck) {
    return "ydke://" + passcodesToBase64(deck.main) + "!" + passcodesToBase64(deck.extra) + "!" + passcodesToBase64(deck.side) + "!";
}

function passcodesToBase64(arr) {
	var buffer = new Uint8Array(arr.length * 4);
...
function isYDKE(ydke, b) {
    if (!ydke.startsWith("ydke://")) {
    	if (b) {
    		errorE("YDKe codes must start with ydke://");
    	}
    	return false;
    }
    var components = ydke.slice("ydke://".length).split("!");
    if (components.length < 3) {
    	errorE("Invalid YDKe code");
        return false;
    }
    return true;
}
...
function base64ToPasscodes(base64) {
	...
	var obj = new Uint32Array(bytes.buffer);
	var arr = [];
...
function exportDeckXML() {
	...
	xml += ' </extra>\n</deck>';
	title = replaceSpecialCharacters(title);
	download(xml, title + ".xml", "text/plain");
}
...
function insertAt0(index, obj, parent) {
...
function insertAt(index, obj, parent) {
	if (parent.hasClass("scrollpane") && parent.find("*[data-overlayscrollbars-viewport]:first").length > 0) {
		parent = parent.find("*[data-overlayscrollbars-viewport]:first");
	}
	if (index == 0) {
		parent.prepend(obj);
		return;
...
function arrowKeyE(e) {
	...
	if ($(':focus').length == 0 || !$(':focus').is(":visible") || $(':focus')[0] == active_scrollpane[0] || inBounds(active_scrollpane) || (active_scrollpane.is(":visible")) && onScreen(active_scrollpane[0])) {
		var amount = (e.which == 40 ? 30 : -30);
		switch(e.which) {
			case 37: // left
			case 39: // right
				amount = (e.which == 39 ? 30 : -30);
				if (fancy_scrollbars) {
					...
					tweenScrollbar(active_scrollpane, 0.08333, {"scrollLeft":active_scrollpane.scrollLeft() + amount, ease:Linear.easeNone});
					return;
				}
				...
				//TweenMax.to(active_scrollpane[0], 0.08333, {"scrollLeft":active_scrollpane[0].scrollLeft + amount});
				tweenScrollbar(active_scrollpane, 0.08333, {"scrollLeft":active_scrollpane[0].scrollLeft + amount, ease:Linear.easeNone});
				break;
			case 38: // up
			case 40: // down
				if (fancy_scrollbars) {
				...
				tweenScrollbar(active_scrollpane, 0.08333, {"scrollTop":active_scrollpane.scrollTop() + amount, ease:Linear.easeNone});
				break;
		}
	}
}
...
function initDB() {
	...
	//$('input').click(function(){ // for super_cb // 7-3-23 I don't see a difference. disabling.
	//	$(this).focus();
	//});
	...
	//setTimeout(addScrollBars, 450);
	var millis1 = new Date().getTime();
	addScrollBars();
	var millis2 = new Date().getTime();
	console.log('millis = ' + (millis2 - millis1));
	...
	var millisA = new Date().getTime();
	changeComponents();
	var millisB = new Date().getTime();
	console.log('millis = ' + (millisB - millisA));
	hideDisplayBoxes();
...
function tweenScrollbar0(scrollpane, time, obj) {
	var proxy = {};
...
function tweenScrollbar(scrollpane, time, obj) {
	if (fancy_scrollbars) {
		TweenMax.to(scrollpane.data("scrollpane").elements().viewport, time, obj);
		return;
	}
	TweenMax.to(active_scrollpane[0], time, obj);
}

function tweenScrollbar(scrollpane, time, obj) {
	TweenMax.to(fancy_scrollbars ? scrollpane.data("scrollpane").elements().viewport : active_scrollpane[0], time, obj);
}
...
function stopGif(i) {
	...
	var internval = setInterval(function(){
		if (i.complete) {
			...
			try {
				//i.crossOrigin = "anonymous"; // does nothing (except turn .com into .comwww.)
...
function increaseUsers() {
	for (var i = 0; i < 10000; i++) {
		onData({"action":"Online user","username":i + "Krushernator"});
	}
}
increaseUsers();

function increaseAllUsers() {
	var obj = {};
	obj.action = "Online users";
	var arr = [];
	for (var i = 0; i < 90000; i++) {
		arr.push({"action":"Online user","u":i.toString()});
	}
	obj.users = arr;
	onData(obj);
}
...
$('.cell:contains("6")').addClass("has6");
$('.cell:contains("9")').addClass("has9");
...
function getZoom() {
	var zoom = window.outerWidth * window.devicePixelRatio / window.outerWidth;
	if (SAFARI) {
		zoom = window.outerWidth / window.visualViewport.width;
	}
	else if (MAC) {
		zoom = window.outerWidth * (window.devicePixelRatio / 2) / window.outerWidth;
	}
	zoom = Math.round(zoom * 100);
	console.log('Zoom = ' + zoom);
	//return zoom;
	return ~~zoom; // to help prevent NaN
}
...
function parse(str) {
	try {
		...
		return {"action":"Error", "message":"Malformed response"};
	}
}
