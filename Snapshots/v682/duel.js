...
addDragging($('#watch_chat'), ".chat_top_bg", ".chat_background", function(){
	if (!fancy_scrollbars || $('#watch_chat .cout_txt').scrollTop() == 0) {
...
addDragging($('#duel_log'), ".chat_top_bg", ".chat_background", function(){
	if (!fancy_scrollbars || $('#duel_log .log_txt').scrollTop() == 0) {
		appendChat($('#duel_log'));
	}
}, function(){
	toggleAds($('#duel_log'));
...
function initDecks(data) {
	...
	if (tag_duel) {
		...
		for (i = 0; i < data.player3.main_total; i++) {
			...
			card.data("owner", player3);
			card.data("controller", player3);
		...
		for (i = 0; i < data.player4.main_total; i++) {
			...
			card.data("owner", player4);
			card.data("controller", player4);
			if (player1.username == username && !automatic) {
				card.css("cursor", "pointer");
				card.click(targetCard);
			}
...
function detach(player, data) {
	var card = removeXyzMaterial(player, data);
	card.data("isXyzMaterial", false);
	...
	TweenMax.to(card, easeSeconds, {left:card.data("owner").graveX, top:card.data("owner").graveY, rotation:card.data("owner").rot, rotationY:ABOUT_ZERO, ease:Linear.easeNone, onComplete:function(){
		...
		//endAction(); // disabling because endAction is also called in updateXyzMaterials which is called in removeXyzMaterial
	}});
	playSound(Move);
}
...
function updateXyzMaterials(player, card, end) {
	if (end !== false) {
		end = true;
	}
	var xStart = 0;
	...
	for (var i = card.data("xyz_arr").length - 1; i > -1; i--) {
		...
		$('#field').append(material);
		if (player == player1 || player == player3) {
			TweenMax.to(material, easeSeconds, {left:xStart + ((27 / (card.data("xyz_arr").length - overlayingInDef)) * (i + 1 - overlayingInDef) - oneMaterial), top:yPosition, scale:0.1485, rotation:player.rot, rotationY:ABOUT_ZERO, z:card.data("xyz_arr").length - i, ease:Linear.easeNone});
		}
		else {
			TweenMax.to(material, easeSeconds, {left:xStart - ((27 / (card.data("xyz_arr").length - overlayingInDef)) * (i + 1 - overlayingInDef) - oneMaterial), top:yPosition, scale:0.1485, rotation:player.rot, rotationY:ABOUT_ZERO, z:card.data("xyz_arr").length - i, ease:Linear.easeNone});
		}
	...
	TweenMax.to(card, easeSeconds, {left:overlayingX, top:yPosition, scale:0.1485, ease:Linear.easeNone, onComplete:function(){
		if (end) {
			endAction();
		}
	}});
