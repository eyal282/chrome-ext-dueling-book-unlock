...
function tweenSleeves() {
	...
	for (var i = 0; i < sleeve_images_arr.length; i++) {
		...
		TweenMax.fromTo(sleeves_arr[i], 0.7, {left:sleeveX + 200, top:sleeveY + 100, alpha:0}, {left:sleeveX, top:sleeveY, alpha:1, ease:Expo.easeInOut, delay:0.01 * i * Math.random()});
...
function rankVideoE() {
	...
	//var arr = ["0", "1", "2", "3"];
	var arr = ["3", "2", "1", "0"];
	getComboBox("Rank Video", "Select a rank for this video: ", arr, arr.indexOf($(this).data("p").toString()), function(){
...
function deleteCardE() {
	if (uploader >= 2 || moderator >= 2) {
		var options = ["Delete Card", "Change Card", "Count Decks"];
